(() => {
    ...
    return `
      <div class="diagnostic-student-jump print-hidden">
        <div class="diagnostic-student-jump-copy">
          ...
          <strong>Ir direto para o card do aluno</strong>
          <small>Selecione um nome para navegar até as informações individuais.</small>
        </div>
        <label class="diagnostic-student-jump-control" for="diagnostic-student-select">
          <span>Aluno</span>
          <select id="diagnostic-student-select" aria-label="Ir para o card de um aluno">
  ...
  function diagnosticInstrumentStudentCards(students = []) {
    ...
    const sorted = [...students].sort((a, b) => {
      const initialYearA = diagnosticInitialYear(a.serie) ?? 999;
      const initialYearB = diagnosticInitialYear(b.serie) ?? 999;
      if (initialYearA !== initialYearB) return initialYearA - initialYearB;
    ...
    sorted.forEach((student) => {
      const initialYear = diagnosticInitialYear(student.serie);
      const label = initialYear
        ? `${initialYear}º ano`
        : (String(student.serie || "Outros anos / séries").trim() || "Outros anos / séries");
      const key = initialYear ? `initial-${initialYear}` : `other-${diagnosticTextKey(label)}`;
    ...
    return groups.map((group) => `
      <section class="instrument-year-group">
        <header class="instrument-year-header">
          <strong>${escapeHtml(group.label)}</strong>
          <span>${group.students.length} ${group.students.length === 1 ? "aluno" : "alunos"}</span>
        </header>
        <div class="instrument-year-student-grid">
