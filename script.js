(() => {
  ...
  const AUTH_EMAIL_DOMAIN = "pme-carbonita.local";
  ...
  function profileFromRow(profile) {
    if (!profile?.username || !profile?.role) {
      const error = new Error("O usuário autenticou, mas não possui um perfil PME válido.");
      error.code = "PME_PROFILE_MISSING";
      throw error;
    }

    if (profile.active === false) {
      const error = new Error("Este acesso está desativado. Procure a coordenação do PME.");
      error.code = "PME_PROFILE_DISABLED";
      throw error;
    }
    ...
    return {
      ...
      nome: profile.nome || profile.username
    };
  }
  ...
  async function loadCurrentProfile(userId) {
    const { data, error } = await supabaseClient
      .from("pme_usuarios")
      .select("user_id, username, nome, role, institution_id, active")
      .eq("user_id", userId)
      .maybeSingle();
    ...
    if (!data) {
      const missing = new Error("O login existe no Supabase Auth, mas ainda não está vinculado à tabela pme_usuarios.");
      missing.code = "PME_PROFILE_MISSING";
      throw missing;
    }
  ...
  function friendlyLoginError(error) {
    ...
    if (/failed to fetch|network|load failed|fetch/.test(message)) {
      return {
        kind: "network",
        message: "Não foi possível conectar ao Supabase. Verifique sua internet e tente novamente."
      };
    }

    if (
      code === "PME_PROFILE_MISSING" ||
      /pme_usuarios|perfil pme|não está vinculado|does not exist|relation .* does not exist/.test(message)
    ) {
      return {
        kind: "config",
    ...
    if (code === "PME_PROFILE_DISABLED") {
      return {
        kind: "credentials",
        message: "Este acesso está desativado. Procure a coordenação do PME."
      };
    }
    ...
    if (/email not confirmed/.test(message)) {
      return {
        kind: "config",
        message: "Este usuário ainda não foi confirmado no Supabase Auth. Confirme o e-mail no painel do Supabase."
      };
    }

    if (
      status === 400 ||
      /invalid login credentials|invalid credentials|invalid email or password|user not found/.test(message)
    ) {
      return {
        kind: "credentials",
    ...
    if (status === 429 || /rate limit|too many requests/.test(message)) {
      return {
        kind: "network",
        message: "Muitas tentativas de acesso. Aguarde um instante e tente novamente."
      };
    }

    return {
      kind: "config",
      message: error?.message || "Não foi possível concluir o acesso."
    };
  }
  ...
  async function startAuthenticatedSession(session) {
    ...
    try {
      ...
      updatePrintDate();
      setSaveState("Carregando relatórios…", true);
      await loadReports();
      setSaveState("Pronto para salvar");
    } catch (error) {
      console.error("[PME] Falha ao carregar perfil:", error);
      ...
      showAuth(friendly.message);
      elements.loginError.dataset.kind = friendly.kind;
      throw error;
    }
  }
  ...
  async function handleLogin(event) {
    ...
    try {
      ...
      console.error("[PME] Erro de autenticação:", error);
  ...
  async function handleLogout() {
    ...
    try {
      ...
      console.warn("[PME] Não foi possível encerrar a sessão no servidor:", error);
    } finally {
      reports = [];
      editingId = null;
      ...
      showAuth();
      setLoginHelp("Acesso protegido pelo Supabase Auth.");
    }
  }

  async function loadReports() {
    if (!currentSession?.user) throw new Error("Sessão inválida.");
  ...
  function setSaveState(text, busy = false) {
    ...
    elements.saveReport.disabled = busy || !currentSession?.user;
  }
  ...
  async function saveCurrentReport() {
    ...
    try {
      ...
      const answers = buildResponsePayload().map((item) => ({
      ...
      if (answers.length) {
        const { error: answersError } = await supabaseClient
          .from("pme_respostas")
          .upsert(answers, { onConflict: "report_id,field_id" });

        if (answersError) throw answersError;
      }

      elements.reportStatusBadge.textContent = report.status || progress.status;
      elements.saveReport.innerHTML = '<span aria-hidden="true">✓</span> Salvar alterações';
      setSaveState(`Sincronizado · ${formatDateTime(report.updated_at || new Date().toISOString())}`);
      toast("Relatório salvo com segurança.");
      await loadReports();
    } catch (error) {
      console.error("[PME] Erro ao salvar:", error);

      const message = String(error?.message || "").toLowerCase();
      if (/jwt|session|token|not authenticated/.test(message)) {
        try {
          await supabaseClient.auth.signOut();
        } catch (_) {}
        showAuth("Sua sessão expirou. Entre novamente.");
      ...
      toast(`Não foi possível salvar: ${error?.message || "verifique as permissões do Supabase."}`, "error");
    }
  }
  ...
  async function editReport(id, printAfter = false) {
    ...
    try {
      const { data: answers, error } = await supabaseClient
        .from("pme_respostas")
        .select("field_id, answer, sort_order")
      ...
      console.error("[PME] Erro ao carregar respostas:", error);
  ...
  async function deleteReport(id) {
    try {
      ...
      console.error("[PME] Erro ao excluir:", error);
  ...
  async function exportCsv() {
    ...
    try {
      const { data: rows, error } = await supabaseClient
        .from("pme_base_powerbi")
        .select("*")
        .order("relatorio_id", { ascending: true })
      ...
      const header = [
        ...
        "resposta", "criado_por", "relatorio_criado_em", "relatorio_atualizado_em",
        "resposta_atualizada_em"
      ];
      ...
      console.error("[PME] Erro ao exportar:", error);
    ...
    try {
      ...
      if (data?.session?.user) {
        try {
          ...
          // A função já retorna à tela de login com a mensagem adequada.
        }
      } else {
        showAuth();
        setLoginHelp("Acesso protegido pelo Supabase Auth.");
      ...
      showAuth("Não foi possível conectar ao Supabase.");
      setLoginHelp("Verifique a configuração do projeto e a sua conexão.");
    }
  }
