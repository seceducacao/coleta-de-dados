(() => {
  ...
  const DIAGNOSTIC_ALL_VALUE = "__all__";
  ...
  function populateDiagnosticInstitutions() {
    ...
    const schoolOptions = allowed
      .map((institution) => `<option value="${institution.id}">${escapeHtml(institution.name)}</option>`)
      .join("");

    elements.diagnosticInstitutionSelect.innerHTML = isAdmin
      ? `<option value="${DIAGNOSTIC_ALL_VALUE}">Todas as escolas</option>${schoolOptions}`
      : schoolOptions;

    if (isAdmin) {
      elements.diagnosticSchoolControl.hidden = false;
      elements.diagnosticInstitutionSelect.disabled = false;
      elements.diagnosticInstitutionSelect.value = DIAGNOSTIC_ALL_VALUE;
      elements.diagnosticTitle.textContent = "Diagnóstico consolidado";
    } else {
  ...
  function diagnosticStatusBadge(value) {
    ...
    else if (normalized.includes("constr")) kind = "building";
    else if (normalized.startsWith("nao") || normalized.startsWith("não")) kind = "no";
    return `<span class="diagnostic-status ${kind}">${escapeHtml(raw || "Sem registro")}</span>`;
  }
  ...
  function groupDiagnosticStudents(students = []) {
    ...
    for (const student of sorted) {
      const seriesLabel = diagnosticCanonicalSeries(student.serie, student.__institutionId || elements.diagnosticInstitutionSelect?.value || "");
  ...
  function diagnosticStudentSummaryMeta(student = {}) {
    const institutionId = student.__institutionId || elements.diagnosticInstitutionSelect?.value || "";
    const items = [
      { label: diagnosticSeriesFieldLabel(institutionId), value: diagnosticCanonicalSeries(student.serie, institutionId) || "—" },
      { label: "CID", value: student.cid || "—" }
    ];

    if (elements.diagnosticInstitutionSelect?.value === DIAGNOSTIC_ALL_VALUE && student.__schoolName) {
      items.push({ label: "Escola", value: student.__schoolName });
    }
  ...
  function groupInclusiveStudentsBySeries(students = []) {
    ...
    for (const student of sorted) {
      const label = diagnosticCanonicalSeries(student.serie, student.__institutionId || elements.diagnosticInstitutionSelect?.value || "");
        ...
        <label class="diagnostic-student-jump-control" for="diagnostic-student-select">
          ...
          <div class="diagnostic-student-select-shell">
            ...
            <select id="diagnostic-student-select" aria-label="Localizar aluno no diagnóstico">
              ...
              ${options.map((item) => `<option value="${escapeHtml(item.anchor)}">${escapeHtml(item.name)}${item.series ? ` · ${escapeHtml(item.series)}` : ""}${elements.diagnosticInstitutionSelect?.value === DIAGNOSTIC_ALL_VALUE && item.school ? ` · ${escapeHtml(item.school)}` : ""}</option>`).join("")}
            </select>
          </div>
        </label>
      </div>
  ...
  const INSTRUMENT_FIELDS = {
    ...
    estudo_caso: { label: "Estudo de caso", field: "estudo_caso" }
  };
  ...
  function diagnosticInstrumentStudentCards(students = []) {
    ...
    return students.map((student) => `
      <article class="instrument-student-mini-card">
        ...
        ${elements.diagnosticInstitutionSelect?.value === DIAGNOSTIC_ALL_VALUE && student.__schoolName
          ? `<span class="instrument-student-school">${escapeHtml(student.__schoolName)}</span>`
          : ""}
        <div class="instrument-student-mini-meta">
          <span>
            <small>${escapeHtml(diagnosticEarlyChildhoodStages(student.__institutionId || elements.diagnosticInstitutionSelect?.value || "") ? "Etapa" : "Ano inicial")}</small>
  ...
  function diagnosticInstrumentSummaryFromStudents(students = [], field = "") {
    const result = { sim: 0, construcao: 0, outros: 0 };
    students.forEach((student) => {
      const kind = diagnosticInstrumentStatusKind(student?.[field]);
      if (result[kind] !== undefined) result[kind] += 1;
    });
    return result;
  }

  function buildConsolidatedDiagnosticData(reports = []) {
    const students = reports.flatMap((report) => {
      const institutionId = report.__institutionId || "";
      const schoolName = report?.school?.nome || report.__institutionName || "Escola";
      return (Array.isArray(report?.students) ? report.students : []).map((student) => {
        student.__institutionId = institutionId;
        student.__schoolName = schoolName;
        return student;
      });
    });

    return {
      school: { nome: "Todas as escolas" },
      summary: {
        total: students.length,
        pei: diagnosticInstrumentSummaryFromStudents(students, "pei"),
        pdi: diagnosticInstrumentSummaryFromStudents(students, "pdi"),
        pai: diagnosticInstrumentSummaryFromStudents(students, "pai"),
        estudo_caso: diagnosticInstrumentSummaryFromStudents(students, "estudo_caso")
      },
      students,
      reports
    };
  ...
  function diagnosticSchoolSummaryCard(report = {}) {
    const students = Array.isArray(report.students) ? report.students : [];
    const separated = splitDiagnosticStudents(students);
    const schoolName = report?.school?.nome || report.__institutionName || "Escola";

    return `
      <article class="consolidated-school-summary-card">
        <div class="consolidated-school-summary-title">
          <span class="eyebrow">Escola</span>
          <strong>${escapeHtml(schoolName)}</strong>
        </div>
        <div class="consolidated-school-summary-stats">
          <span><b>${students.length}</b> alunos</span>
          <span><b>${separated.withSupport.length}</b> Educação Especial</span>
          <span><b>${separated.inclusive.length}</b> Educação Inclusiva</span>
        </div>
      </article>
    `;
  }

  function diagnosticSchoolDetailedSection(report = {}) {
    ...
    return `
      <section class="consolidated-school-report">
        <header class="consolidated-school-report-header">
          <div>
            <span class="eyebrow">Relatório por escola</span>
            <h3>${escapeHtml(school.nome || report.__institutionName || "Escola")}</h3>
          </div>
          <span class="consolidated-school-total">${students.length} ${students.length === 1 ? "aluno" : "alunos"}</span>
        </header>

        <section class="diagnostic-identification consolidated-identification">
          <div>
        ...
        ${diagnosticInitialYearsMarkup(students, institutionId)}

        <section class="diagnostic-metrics consolidated-school-metrics">
          ${diagnosticMetric("Alunos no levantamento", students.length)}
        ...
        <section class="diagnostic-section inclusive-education-section">
          <div class="inclusive-section-heading">
            <div><span class="eyebrow">Educação Inclusiva</span></div>
            <span class="inclusive-total">${inclusiveStudents.length} ${inclusiveStudents.length === 1 ? "aluno" : "alunos"}</span>
          </div>
  ...
  function renderConsolidatedDiagnostic(data) {
    ...
    elements.diagnosticContent.innerHTML = `
      <header class="diagnostic-report-header consolidated-report-header">
        <div>
          <span class="eyebrow">Diagnóstico consolidado da Educação Especial</span>
          <h2>Todas as escolas</h2>
          <p>Plano Municipal de Educação 2026–2036 · Carbonita/MG</p>
        </div>
        <span class="diagnostic-source">${reports.length} ${reports.length === 1 ? "escola" : "escolas"} no consolidado</span>
      </header>

      <section class="diagnostic-metrics consolidated-global-metrics">
        ${diagnosticMetric("Escolas", reports.length, "No consolidado")}
        ${diagnosticMetric("Total de alunos", students.length, "Todos os levantamentos")}
        ${diagnosticMetric("Educação Especial", studentsWithSupport.length, "Com professor de apoio")}
        ${diagnosticMetric("Educação Inclusiva", inclusiveStudents.length, "Sem professor de apoio")}
        ${diagnosticMetric("Professores de apoio", uniqueProfessors)}
      </section>

      <section class="diagnostic-section consolidated-school-summary-section">
        <div class="diagnostic-section-heading">
          <span class="eyebrow">Visão geral</span>
          <h3>Resumo por escola</h3>
        </div>
        <div class="consolidated-school-summary-grid">
          ${reports.map((report) => diagnosticSchoolSummaryCard(report)).join("")}
        </div>
      </section>

      <section class="diagnostic-section">
        <div class="diagnostic-section-heading">
          <span class="eyebrow">Acompanhamento consolidado</span>
          <h3>Análise Individualizada</h3>
        </div>
      ...
      <section class="consolidated-reports-list">
        ${reports.map((report) => diagnosticSchoolDetailedSection(report)).join("")}
      </section>
    `;
  }
  ...
  async function loadDiagnostic() {
    ...
    try {
      if (institutionId === DIAGNOSTIC_ALL_VALUE && currentProfile?.role === "admin") {
        elements.diagnosticTitle.textContent = "Diagnóstico consolidado";
        elements.diagnosticContent.innerHTML = '<div class="diagnostic-loading">Consolidando dados de todas as escolas…</div>';

        const allowed = diagnosticAllowedInstitutions();
        const results = await Promise.all(allowed.map(async (institution) => {
          ...
          return {
            ...data,
            __institutionId: institution.id,
            __institutionName: institution.name,
            students: (Array.isArray(data?.students) ? data.students : []).map((student) => ({
              ...student,
              __institutionId: institution.id,
              __schoolName: data?.school?.nome || institution.name
            }))
          };
        }));

        const consolidated = buildConsolidatedDiagnosticData(results);
        currentDiagnosticData = consolidated;
        renderConsolidatedDiagnostic(consolidated);
        return;
      }

      elements.diagnosticTitle.textContent = currentProfile?.role === "admin"
        ? "Diagnóstico por escola"
        : "Diagnóstico da sua escola";
      ...
      data.__institutionId = institutionId;
      data.__institutionName = institution?.name || data?.school?.nome || "Escola";
      data.students = (Array.isArray(data?.students) ? data.students : []).map((student) => ({
        ...student,
        __institutionId: institutionId,
        __schoolName: data?.school?.nome || institution?.name || "Escola"
      }));
