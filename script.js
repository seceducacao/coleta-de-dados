(() => {
  ...
  function profileFromPayload(payload) {
    ...
    if (!profile?.username || !profile?.role) throw new Error("Perfil de acesso inválido.");
    if (!profile.active) throw new Error("Este acesso está desativado. Procure a coordenação do PME.");
    return profile;
  }
  ...
  async function startCustomSession(token, profilePayload = null) {
    sessionToken = token;
    try {
      const payload = profilePayload || await rpc("pme_validar_sessao", { p_token: token });
  ...
  async function handleLogin(event) {
    ...
    try {
      ...
      if (elements.loginPassword.value.length < 1) throw new Error("Informe a senha.");

      elements.loginUsername.value = username;
      const result = await rpc("pme_login", {
        p_username: username,
        p_password: elements.loginPassword.value
      });

      if (!result?.token) throw new Error("Não foi possível iniciar a sessão.");
      await startCustomSession(result.token, result);
      elements.loginPassword.value = "";
    } catch (error) {
      console.error(error);
  ...
  async function initialize() {
    ...
    try {
      ...
      if (storedToken) await startCustomSession(storedToken);
      else showAuth();
    } catch (error) {
      console.error(error);
