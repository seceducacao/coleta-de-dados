(() => {
  ...
  function updateLiquidTabIndicator() {
    ...
    if (!control || !activeTab) return;

    const controlRect = control.getBoundingClientRect();
    const activeRect = activeTab.getBoundingClientRect();
    control.style.setProperty("--liquid-x", `${activeRect.left - controlRect.left}px`);
    control.style.setProperty("--liquid-width", `${activeRect.width}px`);
  ...
  function animateActiveView(activeView) {
    ...
    window.setTimeout(() => activeView.classList.remove("view-entering"), 460);
  }
