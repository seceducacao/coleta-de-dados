(() => {
  ...
  function buildReportTitle() {
    return "Memorando para diagnóstico educacional";
  }

  function collectForm() {
    const title = buildReportTitle();
    if (elements.reportTitle) elements.reportTitle.value = title;
    return {
      institution: elements.institutionSelect.value,
      title,
      referenceDate: elements.referenceDate ? elements.referenceDate.value : today(),
  ...
  function resetForm(keepInstitution = true) {
    ...
    if (elements.reportTitle) elements.reportTitle.value = buildReportTitle();
    if (elements.referenceDate) elements.referenceDate.value = today();
  ...
  function updatePrintDate() {
    const referenceDate = elements.referenceDate ? elements.referenceDate.value : today();
    elements.printDate.textContent = `Data de referência: ${formatDate(referenceDate)}`;
  }

  async function saveCurrentReport() {
    const data = collectForm();
    if (!data.referenceDate) {
      toast("Não foi possível definir a data de referência.", "error");
      return;
    }
  ...
  function editReport(id, printAfter = false) {
    ...
    if (elements.reportTitle) elements.reportTitle.value = report.title || buildReportTitle();
    if (elements.referenceDate) elements.referenceDate.value = report.referenceDate || today();
  ...
  function renderArchive() {
    ...
    const filtered = reports.filter((report) => {
      ...
      const reportTitle = report.title || buildReportTitle();
      return `${reportTitle} ${report.responsible || ""} ${institution.name}`.toLocaleLowerCase("pt-BR").includes(query);
    });
    ...
    elements.archiveContent.innerHTML = `<div class="report-list">${filtered.map((report) => {
      ...
      const reportTitle = report.title || buildReportTitle();
      return `<article class="report-card"><div class="report-card-icon" aria-hidden="true">▤</div><div class="report-card-main"><div class="report-card-title"><h2>${escapeHtml(reportTitle)}</h2><span class="badge">${completed}/${institution.fields.length} campos</span></div><p>${escapeHtml(institution.name)}</p><div class="report-card-meta"><span>▣ ${formatDate(report.referenceDate)}</span>${report.responsible ? `<span>● ${escapeHtml(report.responsible)}</span>` : ""}</div></div><div class="report-card-actions"><button class="button secondary" data-action="edit" data-id="${report.id}">Editar</button><button class="button ghost" data-action="print" data-id="${report.id}" aria-label="Imprimir ${escapeHtml(reportTitle)}">⎙</button><button class="button ghost danger-ghost" data-action="delete" data-id="${report.id}" aria-label="Excluir ${escapeHtml(reportTitle)}">⌫</button></div></article>`;
    }).join("")}</div>`;
  }
  ...
  function bindEvents() {
    ...
    if (elements.referenceDate) elements.referenceDate.addEventListener("change", updatePrintDate);
