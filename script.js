(() => {
  ...
  function populateDiagnosticInstitutions() {
    ...
    if (isAdmin) {
      ...
      // A escola não recebe lista com outras instituições e não possui seletor visível.
  ...
  function diagnosticSeriesRank(value = "") {
    ...
    // Regra principal solicitada: Ensino Fundamental sempre em ordem,
    // começando pelo 1º Ano Fundamental - Anos Iniciais.
    const fundamental = normalized.match(/\b([1-9])\s*[º°o]?\s*ano\s+fundamental/);
    if (fundamental) return Number(fundamental[1]) * 100;

    // Para escolas de Educação Infantil, mantém uma ordem pedagógica coerente
    // quando não houver séries do Ensino Fundamental.
  ...
  function diagnosticProfessorName(student = {}) {
    ...
    return semProfessor ? "Sem professor de Educação Especial" : raw;
  }
  ...
  function groupDiagnosticStudents(students = []) {
    const sorted = [...students].sort((a, b) => {
      ...
      const professorDifference = professorA === "Sem professor de Educação Especial"
        ? 1
        : professorB === "Sem professor de Educação Especial"
          ? -1
          : professorA.localeCompare(professorB, "pt-BR", { sensitivity: "base" });

      if (professorDifference !== 0) return professorDifference;

      return String(a.nome_aluno || "").localeCompare(String(b.nome_aluno || ""), "pt-BR", { sensitivity: "base" });
    });
    ...
    for (const student of sorted) {
      ...
      const professorKey = diagnosticTextKey(professorLabel);
  ...
  function diagnosticStudentRows(students = []) {
    return students.map((student) => `
      <tr>
        <td class="student-name">${escapeHtml(student.nome_aluno || "")}</td>
        <td>${diagnosticStatusBadge(student.laudo)}</td>
        <td>${escapeHtml(student.cid || "—")}</td>
        <td class="diagnostic-description">${escapeHtml(student.descricao_laudo || "—")}</td>
        ...
        <table class="diagnostic-table">
          <thead>
            <tr>
              ...
              <th>Laudo</th>
              <th>CID</th>
              <th>Descrição do Laudo</th>
              <th>Série que estuda</th>
              <th>PEI</th>
              <th>PDI</th>
              <th>PAI</th>
          ...
          <tbody>
            ${diagnosticStudentRows(students) || '<tr><td colspan="9" class="diagnostic-empty-cell">Nenhum estudante registrado.</td></tr>'}
          </tbody>
        </table>
      </div>
    `;
            ...
            <article class="diagnostic-professor-group">
              <header class="diagnostic-professor-header">
                <div class="diagnostic-professor-title">
                  <span class="diagnostic-professor-icon" aria-hidden="true">●</span>
                  <div>
  ...
  function renderDiagnostic(data) {
    ...
    const uniqueProfessors = new Set(
      students
        ...
        .filter((name) => name !== "Sem professor de Educação Especial")
    ...
    elements.diagnosticContent.innerHTML = `
      ...
      <section class="diagnostic-metrics">
        ${diagnosticMetric("Alunos no levantamento", Number(summary.total || 0))}
        ${diagnosticMetric("Com laudo", Number(summary.com_laudo || 0))}
        ${diagnosticMetric("Sem laudo", Number(summary.sem_laudo || 0))}
        ${diagnosticMetric("Professores de Educação Especial", uniqueProfessors)}
      </section>
      ...
      <section class="diagnostic-section students-section">
        <div class="diagnostic-section-heading">
          <span class="eyebrow">Estudantes</span>
          <h3>Alunos por série e professor</h3>
          <p>As séries aparecem em ordem crescente, iniciando pelo 1º Ano Fundamental – Anos Iniciais quando houver. Dentro de cada série, os alunos estão separados pelo professor de Educação Especial.</p>
        </div>

        <div class="diagnostic-student-groups">
          ${diagnosticGroupedStudentsMarkup(students)}
        </div>
      </section>
    `;
  }
