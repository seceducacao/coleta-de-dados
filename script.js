(() => {
  ...
  const elements = {
    ...
    diagnosticStudentControl: document.querySelector("#diagnostic-student-control"),
    diagnosticStudentSelect: document.querySelector("#diagnostic-student-select"),
  ...
  function populateDiagnosticInstitutions() {
    ...
    if (isAdmin) {
      ...
      elements.diagnosticDescription.textContent = "Acesso administrativo aos diagnósticos das escolas municipais.";
      elements.diagnosticPrivacyNote.textContent = "🔒 A administração pode consultar as escolas individualmente.";
    } else {
  ...
  function groupDiagnosticStudents(students = []) {
    ...
    for (const student of sorted) {
      const seriesLabel = String(student.serie || "Série não informada").trim() || "Série não informada";
  ...
  function diagnosticStudentSummaryMeta(student = {}) {
    const items = [
      { label: "Série", value: student.serie || "—" },
  ...
  function diagnosticStudentCard(student = {}) {
    return `
      <article class="diagnostic-student-card">
  ...
  function groupInclusiveStudentsBySeries(students = []) {
    ...
    for (const student of sorted) {
      const label = String(student.serie || "Série não informada").trim() || "Série não informada";
  ...
  function diagnosticInitialYearsMarkup(students = []) {
    ...
    return `
      <section class="diagnostic-section initial-years-section">
        <div class="diagnostic-section-heading initial-years-heading">
          ...
          <p>Quantidade de alunos do levantamento desta escola, distribuída do 1º ao 5º ano.</p>
        </div>
  ...
  function populateDiagnosticStudentFilter(students = [], reset = false) {
    if (!elements.diagnosticStudentControl || !elements.diagnosticStudentSelect) return;

    const previousValue = reset ? "" : elements.diagnosticStudentSelect.value;
    const options = students
      .map((student, index) => ({
        index,
        name: titleCasePersonName(student.nome_aluno || "Aluno sem nome") || "Aluno sem nome",
        series: String(student.serie || "").trim()
      }))
      .sort((a, b) => a.name.localeCompare(b.name, "pt-BR", { sensitivity: "base" }));

    elements.diagnosticStudentSelect.innerHTML = [
      '<option value="">Todos os alunos</option>',
      ...options.map((item) => `<option value="${item.index}">${escapeHtml(item.name)}${item.series ? ` · ${escapeHtml(item.series)}` : ""}</option>`)
    ].join("");

    const stillExists = [...elements.diagnosticStudentSelect.options]
      .some((option) => option.value === previousValue);
    elements.diagnosticStudentSelect.value = stillExists ? previousValue : "";
    elements.diagnosticStudentControl.hidden = students.length === 0;
  }

  function filteredDiagnosticStudents(students = []) {
    const value = elements.diagnosticStudentSelect?.value ?? "";
    if (value === "") return students;

    const index = Number(value);
    return Number.isInteger(index) && students[index] ? [students[index]] : students;
  }

  function instrumentCard(title, data = {}) {
    return `<article class="instrument-card">
      <strong>${escapeHtml(title)}</strong>
      <div class="instrument-stats">
        <span><b>${Number(data.sim || 0)}</b> Sim</span>
        <span><b>${Number(data.construcao || 0)}</b> Em construção</span>
        <span><b>${Number(data.outros || 0)}</b> Não/sem registro</span>
      </div>
    </article>`;
  }
  ...
  function renderDiagnostic(data) {
    ...
    const visibleStudents = filteredDiagnosticStudents(students);
    const separated = splitDiagnosticStudents(visibleStudents);
    const studentsWithSupport = separated.withSupport;
    const inclusiveStudents = separated.inclusive;
    const completeSeparated = splitDiagnosticStudents(students);

    const uniqueProfessors = new Set(
      completeSeparated.withSupport
    ...
    elements.diagnosticContent.innerHTML = `
      ...
      ${diagnosticInitialYearsMarkup(students)}

      <section class="diagnostic-metrics">
        ${diagnosticMetric("Alunos no levantamento", Number(summary.total || students.length))}
        ${diagnosticMetric("Educação Especial", completeSeparated.withSupport.length, "Com professor de apoio")}
        ${diagnosticMetric("Educação Inclusiva", completeSeparated.inclusive.length, "Sem professor de apoio")}
      ...
      <section class="diagnostic-section">
        <div class="diagnostic-section-heading">
          ...
          <h3>Instrumentos individualizados</h3>
          <p>Situação registrada para PEI, PDI, PAI e estudo de caso.</p>
        </div>
        <div class="instrument-grid">
          ${instrumentCard("PEI", summary.pei)}
          ${instrumentCard("PDI", summary.pdi)}
          ${instrumentCard("PAI", summary.pai)}
          ${instrumentCard("Estudo de caso", summary.estudo_caso)}
        </div>
      </section>
  ...
  async function loadDiagnostic() {
    ...
    if (elements.diagnosticStudentControl) elements.diagnosticStudentControl.hidden = true;
    if (elements.diagnosticStudentSelect) elements.diagnosticStudentSelect.innerHTML = '<option value="">Todos os alunos</option>';
    ...
    try {
      ...
      populateDiagnosticStudentFilter(Array.isArray(data?.students) ? data.students : [], true);
  ...
  function bindEvents() {
    ...
    elements.diagnosticStudentSelect.addEventListener("change", () => {
      if (currentDiagnosticData) renderDiagnostic(currentDiagnosticData);
    });
