(() => {
  ...
  function showApp() {
    ...
    // O indicador Liquid Glass precisa ser medido somente depois que a topbar
    // estiver realmente visível. Dois frames garantem que o navegador já tenha
    // calculado o layout antes de posicionar a pílula ativa.
    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => {
        updateLiquidTabIndicator({ animate: false });
      });
    });
  }
  ...
  // Algumas RPCs de listagem retornam um único registro como objeto porque
  // normalizeRpcPayload desembrulha arrays de tamanho 1. Para telas que sempre
  // trabalham com listas (histórico, anexos, respostas e exportação), reconvertemos
  // esse objeto para um array sem alterar o comportamento das RPCs de login/salvamento.
  ...
  async function loadAttachments(reportId) {
    ...
    reportAttachments = rpcList(data);
    renderAttachments();
  }
  ...
  let liquidTabAnimation = null;
  let liquidEnergyTimer = null;

  function liquidTabGeometry(tab, control) {
    if (!tab || !control) return null;
    const controlRect = control.getBoundingClientRect();
    const tabRect = tab.getBoundingClientRect();

    // Quando o app ainda está escondido pela tela de login, getBoundingClientRect()
    // retorna largura zero. Não devemos transformar esse estado em uma pílula de
    // 0 px, pois a borda aparece como uma linha vertical no canto esquerdo.
    if (controlRect.width <= 1 || tabRect.width <= 1 || tabRect.height <= 1) return null;

    return {
      x: tabRect.left - controlRect.left,
      width: tabRect.width,
      center: tabRect.left - controlRect.left + (tabRect.width / 2)
    };
  }

  function energizeLiquidControl(control) {
    if (!control) return;
    window.clearTimeout(liquidEnergyTimer);
    control.classList.remove("is-energized");
    void control.offsetWidth;
    control.classList.add("is-energized");
    liquidEnergyTimer = window.setTimeout(() => control.classList.remove("is-energized"), 560);
  }

  function updateLiquidTabIndicator({ previousTab = null, animate = true } = {}) {
    const control = elements.editorTab?.closest?.(".segmented-control");
    const selection = control?.querySelector?.(".liquid-selection");
    const activeTab = control?.querySelector?.(".segment.active:not([hidden])");
    if (!control || !selection || !activeTab) return;

    const next = liquidTabGeometry(activeTab, control);
    const previous = liquidTabGeometry(previousTab, control);
    if (!next) {
      control.classList.remove("liquid-ready");
      selection.style.removeProperty("width");
      selection.style.removeProperty("transform");
      return;
    }

    const reduceMotion = window.matchMedia?.("(prefers-reduced-motion: reduce)").matches;
    const canAnimate = animate && previous && previousTab !== activeTab && !reduceMotion && selection.animate;

    liquidTabAnimation?.cancel?.();
    control.style.setProperty("--liquid-x", `${next.x}px`);
    control.style.setProperty("--liquid-width", `${next.width}px`);
    control.classList.add("liquid-ready");

    if (!canAnimate) {
      selection.style.width = `${next.width}px`;
      selection.style.transform = `translate3d(${next.x}px, 0, 0)`;
      return;
    }

    const travel = next.center - previous.center;
    const direction = Math.sign(travel) || 1;
    const distance = Math.abs(travel);
    const stretch = Math.min(34, Math.max(12, distance * .20));
    const duration = Math.min(560, Math.max(420, 390 + distance * .34));

    /*
      O vidro não “teleporta”: ele alonga na direção do novo segmento,
      comprime levemente ao chegar e volta ao repouso — uma aproximação web
      da transformação fluida vista nos controles Liquid Glass.
    */
    const frame1X = previous.x + travel * .18 - (direction > 0 ? 0 : stretch * .58);
    const frame2X = previous.x + travel * .72 - (direction > 0 ? stretch * .18 : stretch * .74);
    const frame3X = next.x + direction * 2.5;

    const frame1W = Math.max(previous.width, next.width) + stretch * .60;
    const frame2W = next.width + stretch;
    const frame3W = Math.max(24, next.width - 2.5);

    selection.style.width = `${next.width}px`;
    selection.style.transform = `translate3d(${next.x}px, 0, 0)`;

    liquidTabAnimation = selection.animate([
      {
        width: `${previous.width}px`,
        transform: `translate3d(${previous.x}px, 0, 0) scaleY(1)`,
        filter: "brightness(1) saturate(1)"
      },
      {
        offset: .24,
        width: `${frame1W}px`,
        transform: `translate3d(${frame1X}px, 0, 0) scaleY(.94)`,
        filter: "brightness(1.045) saturate(1.06)"
      },
      {
        offset: .64,
        width: `${frame2W}px`,
        transform: `translate3d(${frame2X}px, 0, 0) scaleY(1.055)`,
        filter: "brightness(1.075) saturate(1.08)"
      },
      {
        offset: .84,
        width: `${frame3W}px`,
        transform: `translate3d(${frame3X}px, 0, 0) scaleY(.985)`,
        filter: "brightness(1.02) saturate(1.03)"
      },
      {
        width: `${next.width}px`,
        transform: `translate3d(${next.x}px, 0, 0) scaleY(1)`,
        filter: "brightness(1) saturate(1)"
      }
    ], {
      duration,
      easing: "cubic-bezier(.20,.88,.22,1)",
      fill: "none"
    });

    energizeLiquidControl(control);
  }

  function animateActiveView(activeView) {
    ...
    window.setTimeout(() => activeView.classList.remove("view-entering"), 330);
  }
  ...
  function showView(view) {
    ...
    const control = elements.editorTab?.closest?.(".segmented-control");
    const previousTab = control?.querySelector?.(".segment.active:not([hidden])") || null;
    ...
    const activeView = editor ? elements.editorView : archive ? elements.archiveView : elements.diagnosticView;
    window.requestAnimationFrame(() => {
      updateLiquidTabIndicator({ previousTab, animate: true });
  ...
  async function editReport(id, printAfter = false) {
    ...
    try {
      ...
      const fields = Object.fromEntries(rpcList(answers).map((item) => [item.field_id, item.answer || ""]));
      editingId = report.id;
  ...
  async function deleteReport(id) {
    try {
      try {
        ...
        for (const attachment of rpcList(attachments)) {
          await rpc("pme_excluir_anexo", {
            p_token: sessionToken,
  ...
  async function exportCsv() {
    ...
    try {
      ...
      const dataRows = rpcList(rows).map((row) => header.map((column) => row[column] ?? ""));
  ...
  function bindEvents() {
    ...
    const liquidSegments = [elements.editorTab, elements.archiveTab, elements.diagnosticTab];
    liquidSegments.forEach((segment) => {
      const release = () => segment.classList.remove("is-pressed");
      segment.addEventListener("pointerdown", () => {
        if (segment.hidden || segment.disabled) return;
        segment.classList.add("is-pressed");
        energizeLiquidControl(segment.closest(".segmented-control"));
      });
    ...
    window.addEventListener("resize", () => window.requestAnimationFrame(updateLiquidTabIndicator));
  ...
  async function initialize() {
    ...
    elements.referenceDate.value = today();
    renderAttachments();
    window.requestAnimationFrame(updateLiquidTabIndicator);
